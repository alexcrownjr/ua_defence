...
def read_file(file_path: Path, parser: str, region=''):
    """"""
...
def write_file(data: list, file_path: str, format: str, region='', write_mode='w'):
    """"""
    _suffix = PurePath(file_path).suffix
    # _file_name = Path(file_path)
    # print(file_path)
    # print(_file_name.name)
    ...
    if args.input_file:
        ...
        try:
            ...
            for _file in _path.glob('*'):
                if _file.is_file():
                    # print(f'input file {_file}  {PurePath(_file).stem}')
                    region_name = translit(PurePath(_file).stem, reversed=True).replace(" ", "_")
                    # print(in_english.replace(" ", "_"))
                    result = read_file(_file, args.parse_rule, region_name)
                    if args.output_file:
                        write_file(result, args.output_file, args.output_file_format, write_mode='a')
                    elif args.output_dir:
                        write_file(result, f'{Path(args.output_dir, region_name)}', args.output_file_format, region_name)
